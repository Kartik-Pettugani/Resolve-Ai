...
export async function processSupportQuery(sessionId, userQuery, confidenceThreshold) {
  const threshold = Number.isFinite(confidenceThreshold) ? confidenceThreshold : getConfidenceThreshold();
  ...
  const outOfScope = /(weather|movie|cricket score|recipe|write code for game)/i.test(userQuery);
  if (outOfScope) {
    const reason = "Out-of-scope query";
    const handoff = buildEscalationSummary(sessionId, reason, [...history, { sender: "user", content: userQuery }]);
    createEscalation(sessionId, "out_of_scope", handoff.summary, handoff.topic);
  ...
  if (geminiKey) {
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `Context:\n${context}\n\nQuestion: ${userQuery}`,
        config: {
          systemInstruction: "You are a customer support AI. Answer only from provided context. If unsure, say you need human escalation.",
          temperature: 0.1
        }
      });
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a customer support AI. Answer only from provided context. If unsure, say you need human escalation."
          },
          {
            role: "user",
            content: `Context:\n${context}\n\nQuestion: ${userQuery}`
  ...
  addMessage(sessionId, "ai", responseText);
  return {
